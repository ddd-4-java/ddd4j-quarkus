...
def managed_versions(path: Path) -> dict[tuple[str, str, str, str], str]:
    ...
    for node in parse(path).findall("dependencyManagement/dependencies/dependency"):
        key = coordinate(node)
...
def main() -> int:
    ...
    quarkus_pom = Path.home() / ".m2/repository/io/quarkus/platform/quarkus-bom" / quarkus_version / f"quarkus-bom-{quarkus_version}.pom"
    with tempfile.TemporaryDirectory(prefix="ddd4j-quarkus-bom-") as temp_dir:
        effective_pom = Path(temp_dir) / "effective-pom.xml"
        subprocess.run([str(repository / "mvnw"), "-q", "-f", str(pom), "help:effective-pom", "-Dverbose", f"-Doutput={effective_pom}"], cwd=repository, check=True)
    ...
    conflicts = [(key, expected, effective[key]) for key, expected in platform.items() if key in effective and effective[key] != expected]
    if conflicts:
    ...
    assertions = {("io.quarkus", "quarkus-core", "jar", ""): quarkus_version, ("org.testcontainers", "testcontainers", "jar", ""): "2.0.5"}
