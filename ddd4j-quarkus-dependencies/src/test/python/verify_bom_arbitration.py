...
def reactor_leaves(repository: Path) -> list[Path]:
    ...
    def visit(pom: Path) -> None:
        ...
        for module in modules:
            ...
            visit(child)
    visit(repository / "pom.xml"); return leaves
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ddd4j-quarkus-bom-") as temp_dir:
        ...
        for leaf in leaves:
            ...
            if value(effective_root, "version") != expected_revision:
                failures.append(f"{leaf.relative_to(repository)}: expected reactor revision {expected_revision}, actual={value(effective_root, 'version')}"); continue
    ...
    print(f"BOM arbitration verified in {len(leaves)} reactor leaves: Quarkus {quarkus_version}, Testcontainers 2.0.5"); return 0
