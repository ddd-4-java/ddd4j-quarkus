...
**Task status:** Tasks 1–3 completed on feature/4.0.x and Task 4 completed on
feature/3.3.x on 2026-09-10. The proposed shared
...
isolated test repository. Feature/3.3.x now carries equivalent observable auth/MQ
behavior while retaining its own Java 17, Maven 3, Quarkus 3.37.4 and ddd4j 2.0
contracts. Dual-branch full gates, publication and CI remain open for Tasks 5–6.
...
Task 4 evidence: the two auth samples retain the same `login/status/me/role/permission/logout`
HTTP journeys as feature/4.0.x, and the three MQ samples retain the same HTTP order
creation, real listener consumption and projection assertions. Kafka/RabbitMQ reuse
`KafkaQuarkusTestResource` and `RabbitMqQuarkusTestResource`; Testcontainers remains
2.0.5. RED evidence recorded Sa-Token `/auth/status` returning 500 before runtime wiring,
and Disruptor timing out before the reviewed ddd4j 2.0 route fix was installed. A fresh
isolated Maven repository then installed ddd4j 2.0 HEAD `cf26fd53` and its BOM,
Disruptor and Kafka artifacts. Java 17 and Java 21 / Maven 3.9.16 verification each
ran the retained auth/MQ samples with `-DskipTests=false`; the five observable sample
tests passed with zero failures/errors/skips. The only 3.3-specific production change
is explicit Jackson creator/property metadata on the three immutable order events,
