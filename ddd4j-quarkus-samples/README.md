...
在仓库根目录使用 Java 21 和 Maven Wrapper（4.0.x 维护线）：

```bash
./mvnw -B -ntp -pl ddd4j-quarkus-samples/ddd4j-quarkus-sample-auth-satoken test
./mvnw -B -ntp -pl ddd4j-quarkus-samples/ddd4j-quarkus-sample-auth-shiro test
./mvnw -B -ntp -DskipTests=false -pl ddd4j-quarkus-samples -am verify
```
